...
exports.network_delete_get = async function(req, res) {
  ...
  try {
    ...
    res.render('network_delete', {title: '删除网络', navigate: navigate, error: '解析网络 ' + req.params.nwid + ' 时出错：' + err});
  }
};
...
exports.ipAssignmentPools = async function(req, res) {
  ...
  req.checkBody('ipRangeStart', 'IP 范围起始必填').notEmpty();
  req.checkBody('ipRangeStart', 'IP 范围起始需要有效的 IPv4 或 IPv6 地址').isIP();
  req.sanitize('ipRangeStart').escape();
  req.sanitize('ipRangeStart').trim();
  req.checkBody('ipRangeEnd', 'IP 范围结束必填').notEmpty();
  req.checkBody('ipRangeEnd', 'IP 范围结束需要有效的 IPv4 或 IPv6 地址').isIP();
  ...
  if (errors) {
    try {
      ...
      res.render('ipAssignmentPools', {title: 'IP 分配池', navigate: navigate, error: '解析网络 ' + req.params.nwid + ' 的详情时出错：' + err});
    }
  } else {
    try {
...
exports.routes = async function (req, res) {
  ...
  if (errors) {
    try {
      ...
      res.render('routes', {title: '路由', navigate: navigate, error: '解析网络详情时出错'});
    }
  } else {
    try {
...
exports.private = async function (req, res) {
  ...
  try {
    ...
    navigate.whence = '/controller/network/' + network.nwid;
    res.render('private', {title: '私有/公共', navigate: navigate, network: network});
  } catch (err) {
    res.render('private', {title: '私有/公共', navigate: navigate, error: '为网络 ' + req.params.nwid + ' 应用私有设置时出错：' + err});
  }
}
...
exports.v4AssignMode = async function (req, res) {
  ...
  try {
    ...
    res.render('v4AssignMode', {title: 'IPv4 分配模式', navigate: navigate, error: '为网络 ' + req.params.nwid + ' 应用 IPv4 分配模式时出错：' + err});
  }
}
...
exports.v6AssignMode = async function (req, res) {
  ...
  try {
    ...
    res.render('v6AssignMode', {title: 'IPv6 分配模式', navigate: navigate, error: '为网络 ' + req.params.nwid + ' 应用 IPv6 分配模式时出错：' + err});
  }
}
...
exports.dns = async function (req, res) {
  ...
  try {
    ...
    res.render('dns', {title: 'DNS', navigate: navigate, error: '更新网络 ' + req.params.nwid + ' 的 DNS 时出错：' + err});
  }
}
...
exports.member_detail = async function(req, res) {
  ...
  try {
    ...
    console.error(err);
    res.render('error', {title: req.params.object, navigate: navigate, error: '解析网络 ' + req.params.nwid + ' 的成员 ' + req.params.id + ' 的详情时出错：' + err});
  }
};
...
exports.easy_get = async function(req, res) {
  ...
  try {
    const network = await zt.network_detail(req.params.nwid);
    res.render('network_easy', {title: '网络简单设置', navigate: navigate, network: network});
  } catch (err) {
    res.render('network_easy', {title: '网络简单设置', navigate: navigate, error: '解析网络 ' + req.params.nwid + ' 的详情时出错：' + err});
  }
}
...
exports.easy_post = async function(req, res) {
  ...
  req.checkBody('networkCIDR', '网络地址必填').notEmpty();
  ...
  if (errors) {
    ...
    res.render('network_easy', {title: '网络简单设置', navigate: navigate, network: network, errors: errors});
  } else {
    try {
      const network = await zt.network_easy_setup(req.params.nwid,
                                                  ...
                                                  ipAssignmentPools,
                                                  v4AssignMode);
      res.render('network_easy', {title: '网络简单设置', navigate: navigate, network: network, message: '网络设置成功'});
    } catch (err) {
      res.render('network_easy', {title: '网络简单设置', navigate: navigate, error: '解析网络 ' + req.params.nwid + ' 的详情时出错：' + err});
    }
  }
...
exports.members = async function(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    req.checkBody('id', '成员 ID 必填').notEmpty();
    ...
    if (req.body.auth) {
      ...
      req.checkBody('activeBridge', '活动桥接状态必须是布尔值').isBoolean();
...
exports.member_delete = async function(req, res) {
  ...
  try {
    ...
    res.render('member_delete', {title: '从网络删除成员', navigate: navigate,
                    error: '解析网络 ' + req.params.nwid + ' 的成员 ' + req.params.id
                              + ' 的详情时出错：' + err});
  }
}
...
exports.assign_ip = async function(req, res) {
  ...
  req.checkBody('ipAddress', 'IP 地址必填').notEmpty();
  req.checkBody('ipAddress', 'IP 地址必须是有效的 IPv4 或 IPv6 地址').isIP();
  req.checkBody('ipAddress', 'IP 地址必须在受管理的路由范围内')
    .custom(value => {
